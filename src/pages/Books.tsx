...
/* ─── Static content (unchanged from user's design) ─── */
...
/* ─── Default purchase links (fallback if API unreachable) ─── */
const FALLBACK_TYRANNY_LINK = "https://www.amazon.com/dp/B0CQPGY91P";
const FALLBACK_CONQUERING_LINK = "https://www.amazon.com/dp/B0CQPGY91P";

/* ─── Component ─── */
const Books = () => {
  const [currentTestimonial, setCurrentTestimonial] = useState(0);

  /* Backend integration — fetch books from /api/books */
  const [tyrannyLink, setTyrannyLink] = useState(FALLBACK_TYRANNY_LINK);
  const [tyrannyImage, setTyrannyImage] = useState<string | null>(null);
  const [conqueringLink, setConqueringLink] = useState(FALLBACK_CONQUERING_LINK);
  const [conqueringImage, setConqueringImage] = useState<string | null>(null);
  ...
  useEffect(() => {
    fetch("/api/books")
      ...
      .then((data: any[]) => {
        ...
        // Match books by title keyword
        const tyranny = data.find((b) =>
          b.title?.toLowerCase().includes("tyranny") || b.title?.toLowerCase().includes("ordinary")
        );
        const conquering = data.find((b) =>
          b.title?.toLowerCase().includes("conquer")
        );

        if (tyranny) {
          if (tyranny.purchaseLink) setTyrannyLink(tyranny.purchaseLink);
          if (tyranny.imageUrl) setTyrannyImage(tyranny.imageUrl);
        }
        if (conquering) {
          if (conquering.purchaseLink) setConqueringLink(conquering.purchaseLink);
          if (conquering.imageUrl) setConqueringImage(conquering.imageUrl);
        }
      })
      .catch(() => {
        // API unreachable locally — fallback links already set
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex justify-center lg:justify-end" data-aos="fade-left" data-aos-delay="200">
              <div className="bg-[hsl(0,0%,90%)] p-8 rounded-lg shadow-2xl">
                <img
                  src={tyrannyImage || bookTyranny}
                  alt="The Tyranny of the Ordinary"
                  className="w-64 md:w-80 rounded shadow-xl"
                  onError={(e) => { (e.target as HTMLImageElement).src = bookTyranny; }}
                />
              </div>
            </div>
          </div>
      ...
      <section className="py-6 bg-background border-b">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
            <a href={tyrannyLink} target="_blank" rel="noopener noreferrer">
      ...
      <section className="relative h-[300px] md:h-[700px] flex items-center justify-center overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
            <div className="space-y-6" data-aos="fade-right">
              ...
              <a href={conqueringLink} target="_blank" rel="noopener noreferrer">
                <Button className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-3 rounded-full">
                  Buy The Book
                </Button>
              </a>
            ...
            <div className="flex justify-center lg:justify-end" data-aos="fade-left" data-aos-delay="200">
              <div className="bg-[hsl(0,0%,85%)] p-8 rounded-lg shadow-2xl">
                <img
                  src={conqueringImage || bookTyranny}
                  alt="Conquering"
                  className="w-64 md:w-80 rounded shadow-xl"
                  onError={(e) => { (e.target as HTMLImageElement).src = bookTyranny; }}
                />
